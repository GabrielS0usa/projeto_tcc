...
public class StatisticsService {
	...
	public UserStatisticsDTO getUserStatistics(Long userId) {
		...
		List<Activity> userActivities = activityRepository.findByUserAndTimestampBetween(user,
	...
	private UserStatisticsDTO aggregateStatisticsFromActivities(List<Activity> activities) {
		...
		ActivityChartsDTO charts = new ActivityChartsDTO(generateWeeklyProgressChart(activities),
				generateCategoryDistribution(activities), generatePerformanceTrends(activities),
	...
	private Map<String, Object> generateWeeklyProgressChart(List<Activity> activities) {
		...
		chart.put("type", "line");
		chart.put("data", activities.stream().collect(Collectors.groupingBy(a -> a.getTimestamp().toLocalDate(),
	...
	private Map<String, Object> generateCategoryDistribution(List<Activity> activities) {
		...
		chart.put("type", "pie");
		chart.put("data",
				activities.stream().collect(Collectors.groupingBy(a -> a.getType().name(), Collectors.counting())));
		return chart;
	}
	...
	private Map<String, Object> generatePerformanceTrends(List<Activity> activities) {
		Map<String, Object> chart = new HashMap<>();
		chart.put("type", "bar");
		chart.put("data", activities.stream().collect(
				Collectors.groupingBy(a -> a.getType().name(), Collectors.averagingDouble(Activity::getValue))));
		return chart;
	}
	...
	private StatisticsSummaryDTO calculateSummary(List<Activity> activities) {
		...
		Map<String, Double> averagesByType = activities.stream().collect(
				Collectors.groupingBy(a -> a.getType().name(), Collectors.averagingDouble(Activity::getValue)));
		Integer averageQualityScore = (int) activities.stream().filter(a -> a.getQualityScore() != null)
	...
	private TrendAnalysisDTO analyzeTrends(List<Activity> activities) {
		...
		List<ActivityTrendDTO> trends = activityRepository.findDailyTrends(activities.get(0).getUser(),
				LocalDateTime.now().minusMonths(1), LocalDateTime.now());
