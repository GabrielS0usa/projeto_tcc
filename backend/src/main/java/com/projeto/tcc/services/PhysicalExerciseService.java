...
public class PhysicalExerciseService {
	...
	public WalkingStartDTO startWalkingSession() {
		...
		walkingSessionRepository.findByUserAndIsActiveTrue(user).ifPresent(session -> {
	...
	public WalkingSessionDTO getActiveWalkingSession() {
		User user = getCurrentUser();
		WalkingSession session = walkingSessionRepository.findByUserAndIsActiveTrue(user).orElse(null);
	...
	public List<WalkingSessionDTO> getAllWalkingSessions() {
		User user = getCurrentUser();
		return walkingSessionRepository.findByUserOrderByStartTimeDesc(user).stream().map(WalkingSessionDTO::new)
	...
	public List<WalkingSessionDTO> getWalkingSessionsByDateRange(LocalDate startDate, LocalDate endDate) {
		...
		return walkingSessionRepository.findByUserAndStartTimeBetweenOrderByStartTimeDesc(user, start, end).stream()
				.map(WalkingSessionDTO::new).collect(Collectors.toList());
	}
	...
	public List<PhysicalActivityDTO> getAllPhysicalActivities() {
		User user = getCurrentUser();
		return physicalActivityRepository.findByUserOrderByDateDesc(user).stream().map(PhysicalActivityDTO::new)
				.collect(Collectors.toList());
	}
	...
	public List<PhysicalActivityDTO> getPhysicalActivitiesByDateRange(LocalDate startDate, LocalDate endDate) {
		...
		return physicalActivityRepository.findByUserAndDateBetweenOrderByDateDesc(user, start, end).stream()
				.map(PhysicalActivityDTO::new).collect(Collectors.toList());
	}
	...
	public DailyExerciseGoalDTO updateDailyGoal(DailyExerciseGoalDTO dto) {
		...
		DailyExerciseGoal goal = dailyExerciseGoalRepository.findByUserAndDate(user, date).orElseGet(() -> {
	...
	public WeeklyExerciseSummaryDTO getWeeklySummary() {
		...
		List<DailyExerciseGoal> weeklyGoals = dailyExerciseGoalRepository.findByUserAndDateBetweenOrderByDateDesc(user,
				weekStart, today);

		int totalSteps = weeklyGoals.stream().mapToInt(DailyExerciseGoal::getCurrentSteps).sum();

		int totalMinutes = weeklyGoals.stream().mapToInt(DailyExerciseGoal::getCurrentMinutes).sum();

		int totalCalories = weeklyGoals.stream().mapToInt(DailyExerciseGoal::getCurrentCalories).sum();

		int activeDays = (int) weeklyGoals.stream().filter(g -> g.getCurrentSteps() > 0 || g.getCurrentMinutes() > 0)
				.count();

		List<DailyExerciseGoalDTO> dailyGoalDTOs = weeklyGoals.stream().map(DailyExerciseGoalDTO::new)
				.collect(Collectors.toList());

		return new WeeklyExerciseSummaryDTO(totalSteps, totalMinutes, totalCalories, activeDays, dailyGoalDTOs);
	}
