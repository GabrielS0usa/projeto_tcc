...
public interface ActivityRepository extends JpaRepository<Activity, Long> {
	...
	List<Activity> findByUserAndTypeAndPeriod(@Param("user") User user, @Param("type") ActivityType type,
	...
	@Query("SELECT new com.projeto.tcc.dto.ActivityTrendDTO(" + "CAST(a.timestamp AS date), AVG(a.value), COUNT(a)) "
			+ "FROM Activity a " + "WHERE a.user = :user AND a.timestamp BETWEEN :start AND :end "
			+ "GROUP BY CAST(a.timestamp AS date) " + "ORDER BY CAST(a.timestamp AS date)")
	List<ActivityTrendDTO> findDailyTrends(@Param("user") User user, @Param("start") LocalDateTime start,
